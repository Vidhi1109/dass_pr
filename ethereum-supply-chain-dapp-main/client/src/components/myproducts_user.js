...
export default function SimpleProductTable(props) {
  ...
  function show_transactions_all(index , product)
  {
    var count2 = []
    for (let i=0 ; i<count.length ; i++)
    {
    ...
    setCount(count2);
    props.show_transactions(product , transaction);     
          ...
          {props.rows.map((row , index) => (
            <TableRow key={row.paymentAddress} >
              ...
              <TableCell align="right">
                <IconButton color="primary" aria-label="add to shopping cart" size="small" onClick={() => show_transactions_all(index , row)}>
                  Show All Transactions &nbsp;<ShoppingCartIcon/>
                </IconButton>
                { count[index]==1 && (
    <div class="dropdown">
      <ul>
      ...
      <div class="some-class" onChange={onChangeValue} >
        {transactions.map((transaction) =>(
            <div>
                <b>From: {transaction.from}</b><br></br>
                <b>To: {transaction.to}</b><br></br>
                <b>Transaction Hash: {transaction.transactionHash}</b><br></br>
                <b>Time Stamp: {transaction.timestamp}</b><br></br>
                <br></br>
                <br></br>
            </div>
        ))}
